...
import { deletePost, editPost, fetchSavedPosts } from '../services/postsService'
...
function SavedPostsPage(){
    ...
    const loadPosts = async () => {
        const postsData = await fetchSavedPosts(loggedInUser.getUserId())
    ...
    if (posts.length === 0) return (
        <div>
            ...
            <button onClick={() => navigate('/profile')} data-testid="savedPostsGoBackButton">Go Back to Profile</button>
        </div>
    )
